    ...
    P: Point,
{
    ...
    /// Creates a new AABB from pre-sorted lower and upper bounds.
    ///
    /// The caller must ensure that `lower` component-wise <= `upper`.
    /// No min/max computation is performed.
    pub fn from_sorted(lower: P, upper: P) -> Self {
        AABB { lower, upper }
    }
