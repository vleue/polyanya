...
impl RecastFullMesh {
    ...
    fn triangles_with_mesh_info(&self) -> Vec<([u32; 3], u8, u16)> {
        self.detailed
            .meshes
            .iter()
            .zip(self.rasterised.areas.iter())
            .zip(self.rasterised.flags.iter())
            .flat_map(|((mesh, area), flags)| {
                self.detailed
                    .triangles
                    .iter()
                    .skip(mesh.first_triangle_index as usize)
...
impl From<RecastFullMesh> for Mesh {
    fn from(full: RecastFullMesh) -> Self {
        ...
        for area in full.rasterised.areas() {
            let mut layer = Layer {
                vertices: full
                    .detailed
                    .vertices
        ...
        detailed_navmesh.remove_useless_vertices();
        detailed_navmesh.reorder_neighbors_ccw_and_fix_corners();
        // #[cfg(not(feature = "no-default-baking"))]
        // detailed_navmesh.bake();
        detailed_navmesh
